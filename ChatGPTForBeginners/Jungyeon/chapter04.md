...
- LLM이 토큰을 생성하는 방식을 떠올리면 문장을 만들 때 문장이 점점 길어져도 이전 토큰에 대한 어탠션 결과는 항상 동일. 왜냐하면 다음 토큰은 찾아아보지 않는 마스크드 어텐션으로 진행되기 때문
...
- 이후 사후 학습을 통해 사용자의 프로젝트를 잘 따르도록 만듦듦 이렇게 만든 모델을 지시 모델이라고 함
...
- DPO는 두개의 응답 중 어느 것을 더 선호하는지에 대한 비교 데이터를 사용용하여 선호하는 응답에 확률을 더 높이는 방식으로 모델 학습
